...
function amendEntry(entry) {
  ...
  if (this.options.externals) {
    this.options.externals.forEach((path) => {
      const hash = exports.HashChunk(path, seen);
...
function compilationHandler(publicPath, compilation, callback) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(HtmlWebpackPlugin, 'getHooks')) {
      ...
      exports.FirePluginEvent(pluginOptions.name, 'htmlWebpackPluginAlterAssetTags', compilation, async (assets) => {
        const res = await updateHtmlTags.call(this, assets.html, assets.head, assets.body, tagFunction);
        assets.head = res.headTags;
        assets.body = res.bodyTags;
      });
      exports.FirePluginEvent(
        pluginOptions.name,
...
function wrappedFn(fn, pluginArgs, callback) {
  fn(pluginArgs)
    .then(() => {
      if (callback) {
...
exports.FirePluginEvent = function (pluginOptions, event, compilation, fn) {
  if (compilation.hooks) {
    if (Object.prototype.hasOwnProperty.call(compilation.hooks, event)) {
      compilation.hooks[event].tapAsync(pluginOptions, wrappedFn.bind(this, fn));
    }
  } else {
    compilation.plugin(event, wrappedFn.bind(this, fn));
  }
};
