...
import { ConvertArrToEnum } from '../ts-helper';
...
>(
  ...
  if (store.actions) {
    res.actionKeys = ConvertArrToEnum(Object.keys(store.actions), (key) => `${name}${key}`) as any;
  }

  if (store.getters) {
    res.getterKeys = ConvertArrToEnum(Object.keys(store.getters), (key) => `${name}${key}`) as any;
  }

  if (store.mutations) {
    res.mutationKeys = ConvertArrToEnum(Object.keys(store.mutations), (key) => `${name}${key}`) as any;
  }
