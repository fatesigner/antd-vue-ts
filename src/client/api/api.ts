...
import { Encrypt } from '../../public/encryption';
import { EncryOptions } from '../../config/encry';
...
Http.interceptors.response.use(
  ...
  function (error) {
    if (typeof error.message === 'string' && /token/i.test(error.message)) {
      ...
      Session.logout();
      // 跳转至授权页
      Vue.prototype.$router.push({ path: Auth.authPath });
...
export class ApiServiceStatic {
  login(params) {
    // 加密 password
    params.password = Encrypt(params.password, EncryOptions.key, EncryOptions.iv);
