<template>
  <Layout class="view-padding menus">
    <div class="vui-mb10">
...
import { IMenu } from '../../../../models/memu';
import { GetVueRegistedRouters } from '../../../../public/common';
import { ExportStreamFile, ImportStreamFile } from '../../../../public/utils';
...
export default class extends Vue {
  ...
  get unbindPages() {
    if (this.tree.nodes) {
      const unbindPages = GetVueRegistedRouters(this.$router);
  ...
  // 导入json文件
  importJsonFile() {
    ImportStreamFile('.json').then((data: any) => {
      /* const d = [];
  ...
  // 导出json文件
  exportJsonFile() {
    ExportStreamFile('system-menus.json', JSON.stringify(this.tree.nodes[0].children, null, 2));
  }
